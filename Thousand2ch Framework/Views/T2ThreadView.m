...
-(void)setThread:(T2Thread *)thread {
	...
	if (_resExtractPath || _resIndexes) [self displayThread];
}
-(T2Thread *)thread { return _thread; }

-(void)setResIndexes:(NSIndexSet *)indexSet {
	setObjectWithRetain(_resIndexes,indexSet);
...
-(void)setHTMLExporter:(id <T2ThreadHTMLExporting_v100>)HTMLExporter {
	...
	if (_HTMLExporter) [self displayThread];
}
...
-(void)setThreadLoaded:(BOOL)aBool { _threadLoaded = aBool; }
...
-(unsigned)resIndexDisplayedOnTop {
	...
	while (deltaY > 0) {
		...
		if (deltaY < docTop) {
			...
			unsigned newResIndex = [resIndexes indexGreaterThanOrEqualToIndex:resIndex + 10];
...
- (void)webView:(WebView *)sender didFinishLoadForFrame:(WebFrame *)frame {
	...
	[_threadView setThreadLoaded:NO];
