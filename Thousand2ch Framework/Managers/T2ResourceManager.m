...
-(void)loadSkinNamed:(NSString *)aString {
	...
	setObjectWithRetain(_skinNames, [skinPathDic allKeys]);
	setObjectWithRetain(_skinFilePath, [skinPathDic objectForKey:aString]);
	if (!_skinFilePath && [skinPathDic count]>0) {
		setObjectWithRetain(_skinFilePath, [[skinPathDic allValues] objectAtIndex:0]);
	} else {
		...
		if (aString) {
			...
			if (iconSetFolder) {
				...
				while (iconFile = [iconFileEnumerator nextObject]) {
					NSImage *icon = [[[NSImage alloc] initByReferencingFile:[iconSetFolder stringByAppendingPathComponent:iconFile]] autorelease];
