...
-(void)setIsBeActive:(BOOL)aBool { 
	if (!_beMail || [_beMail length] == 0 || !_beCode || [_beCode length] == 0) return;
	_isBeActive = aBool;
	...
	if (_isBeActive) {
		if (cookies && ([cookies count] >= 2)) {
			...
			while (cookie = [cookieEnumerator nextObject]) {
				if ([[cookie name] isEqualToString:@"DMDM"]
					&& [[cookie value] isEqualToString:_beMail]) {
					mailFound = YES;
				}
				else if ([[cookie name] isEqualToString:@"MDMD"]
						 && [[cookie value] isEqualToString:_beCode]) {
					codeFound = YES;
				}
			}
			if (mailFound && codeFound) return;
		}
			
		NSDictionary *sharedCookieProperties = [NSDictionary dictionaryWithObjectsAndKeys:
												@".2ch.net", NSHTTPCookieDomain,
												@"/", NSHTTPCookiePath,
												//@"FALSE", NSHTTPCookieSecure, 
												[NSDate dateWithTimeIntervalSinceNow:315576000], NSHTTPCookieExpires, 
												nil];
		
		NSMutableDictionary *beMailCookieProperties = [[sharedCookieProperties mutableCopy] autorelease];
		[beMailCookieProperties setObject:@"DMDM" forKey:NSHTTPCookieName];
		[beMailCookieProperties setObject:_beMail forKey:NSHTTPCookieValue];
		
		NSMutableDictionary *beCodeCookieProperties = [[sharedCookieProperties mutableCopy] autorelease];
		[beCodeCookieProperties setObject:@"MDMD" forKey:NSHTTPCookieName];
		[beCodeCookieProperties setObject:_beCode forKey:NSHTTPCookieValue];
		
		NSHTTPCookie *beMailCookie = [NSHTTPCookie cookieWithProperties:beMailCookieProperties];
		NSHTTPCookie *beCodeCookie = [NSHTTPCookie cookieWithProperties:beCodeCookieProperties];
		
		[sharedHTTPCookieStorage setCookie:beMailCookie];
		[sharedHTTPCookieStorage setCookie:beCodeCookie];
	} else {
...
-(NSArray *)preferenceItems {
	return [NSArray arrayWithObjects:
			...
			/*
			[T2PreferenceItem stringItemWithKey:@"viewerID"
										  title:plugLocalizedString(@"Viewer ID")
										   info:nil],
			[T2PreferenceItem stringItemWithKey:@"viewerPS"
										  title:plugLocalizedString(@"Viewer Password")
										   info:nil],
	 */
			...
			[T2PreferenceItem boolItemWithKey:@"autoLoginViewer"
										title:plugLocalizedString(@"Auto Log-in")
										 info:nil],
			/*
			[T2PreferenceItem stringItemWithKey:@"viewerSID"
										  title:plugLocalizedString(@"Viewer Session ID")
										   info:nil],
			[T2PreferenceItem boolItemWithKey:@"isViewerActive"
										title:plugLocalizedString(@"Activate Viewer")
										 info:nil],
	 */
			[T2PreferenceItem separateLineItem],
...
-(NSURLRequest *)URLRequestForThread:(T2Thread *)thread {
	...
	if (([threadFace state] >= T2ThreadFaceStateFallen || [pathComponents count]>3) && _viewerSUA && _viewerSID) {
...
-(T2LoadingResult)buildThread:(T2Thread *)thread withWebData:(T2WebData *)webData {
	...
	switch (code) {
		...
		case 304: { // Not Modified
			if ([threadFace state] == T2ThreadFaceStateFallen && [threadFace resCountGap] > 0
				&& _viewerSUA && _viewerSID) {
				[thread setValue:@"offlaw" forKey:@"2chViewer"];
				return T2RetryLoading;
			}
...
-(NSURLRequest *)URLRequestForPosting:(T2Posting *)posting {
	...
	if (_isP2Active) {
		...
		NSString *time = @"1";
		if (threadList)
			time = [NSString stringWithFormat:@"%d",[[threadList lastLoadingDate] timeIntervalSince1970]-60*60*60];
...
-(NSURLRequest *)URLRequestForPostingRes:(T2Res *)tempRes thread:(T2Thread *)thread {
	...
	/*
	 ...
	 time = [NSString stringWithFormat:@"%d",[[thread lastLoadingDate] timeIntervalSince1970]];
	 */
	NSString *cgiPath = [NSString stringWithFormat:@"http://%@/test/bbs.cgi",serverName];
...
-(NSURLRequest *)URLRequestForPostingFirstRes:(T2Res *)tempRes threadTitle:(NSString *)threadTitle
	...
	//NSRange boardRange = [boardURLString rangeOfString:[NSString stringWithFormat:@"/%@/",boardKey]];
	NSString *serverName = [boardURLString pathComponentAtIndex:1];
	NSString *time = [NSString stringWithFormat:@"%d",[[threadList lastLoadingDate] timeIntervalSince1970]];
	NSString *cgiPath = [NSString stringWithFormat:@"http://%@/test/bbs.cgi",serverName];
