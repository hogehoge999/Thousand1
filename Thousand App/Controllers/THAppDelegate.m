...
-(id)init {
	...
	[[NSUserDefaults standardUserDefaults] registerDefaults:ThousandDefaultPref];
	...
	NSDictionary *prefDic = [[NSUserDefaults standardUserDefaults] objectForKey:@"appDelegatePref"];
	...
	// Icon Set
	NSDictionary *appDelegatePref = [[NSUserDefaults standardUserDefaults] dictionaryForKey:@"appDelegatePref"];
	NSString *iconSetName = [appDelegatePref objectForKey:@"iconSetName"];
	if (iconSetName)
...
-(void)loadListOperationFinished:(NSNotification *)notification {
	if (!_threadListCache) {
		_threadListCache = [[_loadListOperation lists] mutableCopy];
	}
...
- (void)applicationWillTerminate:(NSNotification *)aNotification {
	...
	NSString *cookiesFilePath = [[[NSString appSupportFolderPath] stringByAppendingPathComponent:@"Thousand"]
								 stringByAppendingPathComponent:@"Thousand Cookies.plist"];
	[[T2HTTPCookieStorage sharedHTTPCookieStorage] saveObjectToFile:cookiesFilePath];
	
	[[NSUserDefaults standardUserDefaults] setBool:YES forKey:@"terminateWithoutCrash"];
}
