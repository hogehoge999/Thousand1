...
-(void)updateThreadMenu {
	...
	[appDelegate setThreadResStyleMenuTitle:
	 [NSString stringWithFormat:NSLocalizedString(@"Style of %@", @"app")
	  ,[[T2PluginManager sharedManager] localizedDescriptionOfExtractPath:[_threadView resExtractPath]]]
									enabled:YES];
...
- (IBAction)printTab:(id)sender {
	...
	[[printInfo dictionary] setObject:@"/Users/k_natori/Desktop/Result.pdf" forKey:NSPrintSavePath]; 
	[[printInfo dictionary] setObject:[NSNumber numberWithInt:2] forKey:NSPrintPagesAcross]; 
	[[printInfo dictionary] setObject:[NSNumber numberWithInt:1] forKey:NSPrintPagesDown]; 
	 
	
	
	NSView *view = [[[_threadView mainFrame] frameView] documentView];
	NSPrintOperation* operation = [NSPrintOperation printOperationWithView:view
															   printInfo:printInfo];
	[operation setShowPanels:NO]; 
	[operation runOperation];
	NSLog(@"printInfo2:%d", [operation printInfo]);
	NSLog(@"%@",[[operation printInfo] dictionary]);
	 */
}
